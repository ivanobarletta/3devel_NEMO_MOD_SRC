...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namlbc in configuration namelist', lwp )
      ...
      REWIND( numnam_ref )              ! Namelist namsol in reference namelist: selected solver for Filtered FSE
      READ  ( numnam_ref, namsol, IOSTAT = ios, ERR = 903 )
903   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namsol_msk in reference namelist', lwp )

      REWIND( numnam_cfg )              ! Namelist namsol in configuration namelist : selected solver for Filtered FSE
      READ  ( numnam_cfg, namsol, IOSTAT = ios, ERR = 904 )
904   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namsol_msk in configuration namelist', lwp )
...
!!gm end
      ...
      !! Do not mask tpol if using PETSc  
      IF ( nn_solv == 3 ) tpol = 1._wp  
