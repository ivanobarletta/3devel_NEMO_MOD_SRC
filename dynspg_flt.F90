...
CONTAINS
   ...
   SUBROUTINE dyn_spg_flt( kt, kindic )
      ...
      END IF  
      IF( neuler == 0 .AND. kt == nit000+1 ) THEN
         CALL sol_mat( kt ) 
         IF (nn_solv == 3 ) CALL petsc_set_mat
      END IF  
      z2dtg  = grav * z2dt
      ...
      IF( lk_vvl ) THEN 
         CALL sol_mat( kt )      ! build the matrix at kt (vvl case only)
         IF (nn_solv == 3 ) CALL petsc_set_mat
      END IF  
      ...
      DO jj = 2, jpjm1
         DO ji = fs_2, fs_jpim1   ! vector opt.
            ...
            gcb2(ji,jj) = zgcb
            !write(6,*) 'gcb ',ji,jj,gcb(ji,jj),gcb2(ji,jj),gcdprc(ji,jj)    
      ...
      IF( nn_solv == 3 ) CALL petsc_set_rhs 
      ...
      IF( ncut == 0 ) THEN
         ...
         !ELSEIF( nn_solv == 3 ) THEN   ;   CALL sol_petsc( kindic )    ! Petsc
         ELSEIF( nn_solv == 3 ) THEN   ;   CALL petsc_solve( kindic )    ! Petsc
         ENDIF
         !CALL sol_petsc( kindic )   
      ENDIF
      ...
      !IF( kt == nitend ) CALL sol_petsc_finalize  
